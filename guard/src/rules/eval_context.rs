...
fn accumulate_map<'value, 'loc: 'value, F>(
    ...
    where F: Fn(usize, &'value [QueryPart<'loc>], &'value PathAwareValue, &dyn EvalContext<'value, 'loc>, Option<&dyn Fn(&str) -> String>) -> Result<Vec<QueryResult<'value>>>
{
    //
    ...
    for each in values {
        ...
        resolved.extend(
            func(query_index+1, query, each, &val_resolver, converter)?)
    }
    Ok(resolved)
}
...
fn check_and_delegate<'value, 'loc: 'value>(conjunctions: &'value Conjunctions<GuardClause<'loc>>)
    -> impl Fn(usize, &'value [QueryPart<'loc>], &'value PathAwareValue, &dyn EvalContext<'value, 'loc>, Option<&dyn Fn(&str) -> String>) -> Result<Vec<QueryResult<'value>>>
{
        ...
        for each in retrieved {
            match each {
                ...
                QueryResult::Resolved(value) => {
                    ...
                    resolved.extend(query_retrieval_with_converter(index, query, value, &scope, converter)?);
                }
            }
        }
        return Ok(resolved)
